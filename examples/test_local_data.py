...
def test_use_local_data():
    # kline = get_local_kline(symbol=['000001'], end='2020-01-30', freq='30min', start='2019-01-01')
    kline = get_local_day_kline('300494', end='2019-12-31', start='2019-01-01')

    ka = KlineAnalyze(kline, name="day", verbose=False)
    print("分型识别结果：", ka.fx_list[-3:])
    print("笔识别结果：", ka.bi_list[-3:])
    print("线段识别结果：", ka.xd_list[-3:])


    zx = find_zs(ka.bi_list)
    print("中枢识别结果：", zx)
